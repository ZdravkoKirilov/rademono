...
export const toExpressionId = (source: unknown) =>
  String(source) as ExpressionId;

type GenericEvent = {};
type StatefulComponent = {};
type DidUpdatePayload = {};

export type Expression = Tagged<
  'Expression',
  BaseModel<ExpressionId> & {
    code: string;
  }
>;
...
export const Expression: GameEntityParser<
  Expression,
  DtoExpression,
  RuntimeExpression
> = {
  ...
  toRuntime(context, expression) {
    return enrichEntity<Expression, RuntimeExpression>(
      context.conf,
      {
...
export type RuntimeExpression<T = ParamedExpressionFunc> = Omit<
  Expression,
  'code'
> & {
  code: T;
...
export type ParamedExpressionFunc<T = {}, ReturnType = any> = (
  payload: T,
) => ReturnType;
