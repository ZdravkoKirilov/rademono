...
export const isDescendantOf = (
  ...
  return (
    isDirectChild ||
    potentialParent.children.some((child) =>
      child ? isDescendantOf(target, child) : false,
    )
  );
};
...
export const findInDescendants = (startPoint: Component) => (
  criteria: Dictionary | RzElementType,
): Component | null => {
...
export const findInAncestors = (startPoint: Component) => (
  criteria: Dictionary | RzElementType,
): Component | null => {
  if (isRzElementType(criteria)) {
    const matcher = matchByType(criteria);
...
export const findInSiblings = (startPoint: Component) => (
  criteria: Dictionary | RzElementType,
): Component | null => {
  if (isRzElementType(criteria)) {
    const matcher = matchByType(criteria);
...
const breadthFirstSearch = (
  ...
  while (queue.length > 0) {
    ...
    if (nextElement?.children) {
      for (let i = 0; i < nextElement.children.length; i++) {
        // next group for checkup
